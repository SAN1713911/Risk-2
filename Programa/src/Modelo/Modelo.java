...
public class Modelo {
    ...
    public void reforzar(String pais) {
        ...
        if(condicion==true){
            int a = (int)(Math.random()*(rojos.size()-0+1)+0);
            rojos.get(a).setSoldados(rojos.get(a).getSoldados()+1);
        }
                ...
                if(dado>azules.size()){
                    for (int i = 0; i < azules.size(); i++) {
                        ...
                        if(azules.get(i).getSoldados()==0){
                            ...
                            ventana.boton5.setEnabled(false);
                        }
    ...
    public void reagrupar(String Atacante){
        ...
        for(int i = 0; i < azules.size(); i++) {
            if(azules.get(i).getNombre().equals(Atacante)) {
                if(azules.get(i).getSoldados()-1>=Integer.parseInt(unidades)){
                    ...
                    ventana.ComboAtacantes.setEnabled(true);
                    ventana.ComboDefensores.setEnabled(true);
                    ventana.boton4.setEnabled(false);
                    ventana.boton5.setEnabled(true);
                    ventana.boton3.setEnabled(true);
    ...
    public void fortificar(String Atacante, String Defensor){
        boolean a = false;
        boolean b = false;
        int azulitos = 0;
        int azulitosjr=0;
        ...
        if(a==true && b==true){
            azules.get(azulitos).setPaises(azules);
            int x = azules.get(azulitos).buscarVecino(azules.get(azulitosjr).toString());
            System.out.println(x);
            if(x!=0){
                String unidades = JOptionPane.showInputDialog("Número de unidades a mover:");
                if(azules.get(azulitos).getSoldados()-1>=Integer.parseInt(unidades)){
                   azules.get(azulitos).setSoldados(azules.get(azulitos).getSoldados()-(Integer.parseInt(unidades)));
                   azules.get(azulitosjr).setSoldados(azules.get(azulitosjr).getSoldados()+(Integer.parseInt(unidades)));
                   for(int i=0; i<azules.size(); i++){
                       azules.get(i).setVisitado(false);
                   }
                   ventana.boton5.setEnabled(false);
                }else{
                   JOptionPane.showMessageDialog(ventana, "No es posible trasladar "+unidades+" soldados");
                   fortificar(Atacante, Defensor);
                }
            }else{
                JOptionPane.showMessageDialog(ventana, "No hay camino");
            }
        }else{
            JOptionPane.showMessageDialog(ventana, "El territorio no le pertenece");
        }
        updateUI();  
    }
